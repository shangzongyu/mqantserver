...
func (t *Table) StartGame(session gate.Session) error {
	playerImp := self.GetBindPlayer(session)
	if playerImp != nil {
		player := playerImp.(*objects.Player)
		player.OnRequest(session)
...
玩家加入场景
*/
func (t *Table) Join(session gate.Session) error {
	self.writelock.Lock()
...
玩家押注
*/
func (t *Table) Stake(session gate.Session, target int64) error {
	playerImp := self.GetBindPlayer(session)
	if playerImp != nil {
		player := playerImp.(*objects.Player)
		player.OnRequest(session)
